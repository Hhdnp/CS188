...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict = {NODE: [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)] for NODE in variables[:3]}
    for obs in [OBS1, OBS0]:
        variableDomainsDict[obs] = list(range(manhattanDistance((0, 0), (X_RANGE - 1, Y_RANGE - 1)) + MAX_NOISE + 1))
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for joinedVariable in eliminationOrder:
            factors, joinedFactors = joinFactorsByVariable(factors, joinedVariable)
            if len(joinedFactors.unconditionedVariables()) > 1:
                eliminatedFactor = eliminate(joinedFactors, joinedVariable)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        sampleDict = self.copy()
        sampleDict.normalize()
        sampleNumber = random.random()

        tmp = 0.0
        for key, prob in sampleDict.items():
            if tmp <= sampleNumber < tmp + prob:
                return key
            else:
                tmp += prob
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        for possibleGhostPosition in self.allPositions:
            prob = self.getObservationProb(observation, pacmanPosition, possibleGhostPosition, jailPosition)
            self.beliefs[possibleGhostPosition] *= prob
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for possiblePosition in self.allPositions:
            newPosDict = self.getPositionDistribution(gameState, possiblePosition)
            oldPositionProb = self.beliefs[possiblePosition]
            for newPosition in newPosDict.keys():
                newPositionProb = newPosDict[newPosition]
                newBeliefs[newPosition] += oldPositionProb * newPositionProb
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        for i in range(self.numParticles):
            id = i % len(self.legalPositions)
            self.particles.append(self.legalPositions[id])
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        jailPosition = self.getJailPosition()
        sampleDict = self.getBeliefDistribution()
        for position in self.allPositions:
            sampleDict[position] *= self.getObservationProb(observation, pacmanPosition, position, jailPosition)
        if sampleDict.total() == 0:
            self.initializeUniformly(gameState)
            return
        sampleDict.normalize()
        self.particles = [sampleDict.sample() for _ in range(self.numParticles)]
        "*** END YOUR CODE HERE ***"
    ...
    def elapseTime(self, gameState):
        ...
        for i in range(self.numParticles):
            newPositionDict = self.getPositionDistribution(gameState, self.particles[i])
            newParticles.append(newPositionDict.sample())
        self.particles = newParticles

        # self.particles = [self.getPositionDistribution(gameState, oldPosition).sample()
        #                   for oldPosition in self.numParticles]
        "*** END YOUR CODE HERE ***"
