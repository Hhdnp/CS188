    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i + 1]]
        "*** YOUR CODE HERE ***"
        maxBeliefs = [max(dist, key=lambda x: dist[x]) for dist in livingGhostPositionDistributions]
        minValue = float('inf')
        minAction = None
        for action in legal:
            value = min(self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), ghostPosition)
                        for ghostPosition in maxBeliefs)
            if value < minValue:
                minValue = value
                minAction = action
        return minAction
