...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        while self.iterations > 0:
            self.iterations -= 1
            newValueList = util.Counter()
            updateFlag = util.Counter()
            for state in self.mdp.getStates():
                bestAction = self.computeActionFromValues(state)
                if bestAction:
                    newValue = self.computeQValueFromValues(state, bestAction)
                    newValueList[state] = newValue
                    updateFlag[state] = 1
            for state in self.mdp.getStates():
                if updateFlag[state]:
                    self.values[state] = newValueList[state]
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for nextState, transitionProbabilities in transitionList:
            qValue += (transitionProbabilities *
